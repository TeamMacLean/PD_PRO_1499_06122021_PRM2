---
title: "007 Heatmap"
author: "Dan MacLean and Clara Jegousse"
date: "25/01/2022"
...
Load the list of peptides with experimental data in the literature according to Neftaly.
...
Find this peptides in the matrix of peptides with significantly different fold changes.

```{r}
exp_targets = data.frame(
  id = rownames(sig_fc_mat) %>% as.vector(),
  selected = stringr::str_split(rownames(sig_fc_mat), " ", simplify = TRUE, n = 2)[,1] %in% exp_peptides_data$`Gene ID`)
```

Out of the 17 peptides with experimental data in the literature, only 9 are in the matrix.

```{r}
exp_targets_in_sig_fc_mat = exp_targets %>% dplyr::filter(selected == TRUE) %>% 
  dplyr::mutate(gene_id = stringr::str_split(id, " ", simplify = TRUE, n = 2)[,1]) %>% 
  dplyr::select(gene_id)

exp_targets_in_sig_fc_mat %>% unique() %>% dim()
```

List the peptides with experimental data in the literature that are NOT in the matrix

```{r}
exp_peptides_data %>%
  dplyr::filter(`Gene ID` %in% exp_targets_in_sig_fc_mat$gene_id == FALSE)
```

Subset the matrix to keep only the peptides with experimental data.

```{r}
selection = exp_targets %>% dplyr::filter(selected == TRUE) %>% dplyr::select(id)
select_sig_fc_mat = sig_fc_mat[selection$id,]
```
...
```{r}
graphics_path = paste(here::here(), "graphics", sep = "/")
plot_filename = paste(graphics_path, "007-prm-pep-exp-validation-barplots.pdf", sep = "/")

ggsave(plot_filename, plot = last_plot(), width = 11, height = 8)
```
