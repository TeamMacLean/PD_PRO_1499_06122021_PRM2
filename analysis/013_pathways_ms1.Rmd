...
To retrieve the phosphorylation values (scaled abundance) of
phosphopeptides found for all of these proteins in the MS1 dataset at
all time points.

To plot small heatmaps for each pathway with all the phosphopeptides
found for their proteins in the strains Guy11 and Dpmk1
...
Remove the list of peptides identified by Frank Menke.

```{r}
ms1_curated_filepath = paste0(here::here(), "/data/curated/013_curation_v3/ms1_id_to_remove.csv")
...
The long dataframe needs pivot-ing into a matrix. Scaling is applied
too.
...
ms1_tb <- cleaned %>%
  mutate(sample_time = paste(sample, time, sep="-"),
         id = paste(`Positions in Master Proteins`, 
           ...
           sep = "|")) %>%
  dplyr::select(id, sample_time, value) %>%
...
raw %>% 
  ...
  dplyr::select(id, modif_position_label)
```

### Replace missing values

We can also replace NA with the minimum observed value on the whole
abundance data - before scaling and distance measuring.
...
We set a colour scheme matching the original Excel document in which
each pathway correspond to one colour:
...
Here we draw all the heatmaps using the viridis colour palette and we
combine them for plotting at the end.
...
For a collaborator interested in knowing the phosphorylation of the
proteins he is working with (Steve proteins)
...
Load the list of peptides with experimental data in the literature
according to Neftaly.

```{r}
exp_peptides_filepath = paste0(here::here(), "/data/raw/Experimentally validated phosphorylation sites in appressorium.xlsx") 

exp_peptides_data = readxl::read_xlsx(exp_peptides_filepath)
```

## Prepare the data table

We create a data frame from the matrix of scale abundance values
`m_scaled`.

```{r, warning=FALSE}
ms1_df = m_scaled %>% as.data.frame() %>% 
  mutate(col = rownames(.)) %>%
  tidyr::separate(col, into = c("protein", "sequence", "modification", "aa"), sep = "\\|") %>%
   pivot_longer(., cols = "dpmk1-0":"Guy11-6") %>%
  tidyr::separate(name, into = c("genotype", "timepoint"), sep = "-") %>%
  tidyr::separate(protein, into = c("gene_id", "pos"), sep = " ") %>%
  mutate(short_gene_id = str_remove(gene_id, "T0"))
  
  
```

## Filter selected peptides

We filter the selected peptides (peptides identified from the
literature)

```{r}
ms1_data = ms1_df %>%
  filter(short_gene_id %in% exp_peptides_data$`Gene ID`)
```

## Plots

For each gene id (MGG number), we select the scaled abundance value as a
data frame `df` and for each peptide corresponding to this MGG number,
we display the barplot.

```{r, message=FALSE, warning=FALSE}
for(g in unique(ms1_data$short_gene_id %>% unique())){
  #print(g)
  
  plot_list = list()
  
  df = ms1_data %>% filter(short_gene_id == g) %>%
    mutate(genotype = factor(genotype, levels = c('Guy11', 'dpmk1')))
  
  for(p in unique(df$sequence)){
    # print(p)
    
    plot = df %>% filter(sequence == p) %>%
    ggplot(.) +
    geom_col(aes(x = as.factor(timepoint), 
     y = value, 
     fill = genotype), 
    position = "dodge") +
      labs(title = g, 
        ...
        y = "scaled abundance") +
      lims(y = c(0,1)) +
      scale_fill_manual(values = c("#FDE725FF", "#414487FF"),
        name = 'Genotypes',
  ...
  pdf_filename = here::here("graphics", paste0("013-ms1-", g, "-bar-plots.pdf"))
  ggexport(plot_list, filename = pdf_filename,
    width = 6, height = 4)
}
```
