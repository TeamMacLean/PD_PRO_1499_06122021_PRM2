...
ms1_tb <- cleaned %>%
  mutate(sample_time = paste(sample, time, sep="-"),
         id = paste(`Positions in Master Proteins`, `Annotated Sequence`, `Modifications`, `Label Modifications in Master Proteins`,  sep = "|")) %>%
...
for (p in unique(pathways$pathway)) {
  ...
  matrix <- mdata$data
   ...
   rownames(matrix) <- paste(mdata$row_info[,'protein_name'],
   str_split( mdata$row_info[,'m_rownames'], "\\|", n = Inf, simplify = TRUE)[,1] ,
