...
## Heatmap on whole data set, but recoloured

Frank would like the same analysis as in doc `007_heatmap.Rmd` but in the `viridis` or similar colour scheme that was used in some other analysis to be in the same paper.  This document is therefore a complete copy of that with only the palette switched.

Ostensibly `viridis` is preferred because the red/blue previously used had a red component that 'could not be seen by some people' ?! The RdBu palette is also colour-blind safe and perceptually uniform so could be used and will remain the default in the package. To satisfy the request I tried interpolating the blue and green shades in `viridis` as well as the purple and yellow, but the nature of the data meant that the overall effect was green, which didn't reach the goal of unifying the colour scheme at all and fails the aesthetic goal of the re-analysis (!lolz!). I used only purple (high) and yellow (low) in this redoing. 
...
ht <- plot_heatmap(many, metric = "bootstrap_t_fdr", log = TRUE, sig_only = FALSE, padding = c(0.5,0.5,0.5,4) ,lgd_x = 8, lgd_y=1, pal="RdBu" )
...
pal <- colorRampPalette(
  c(
    rgb(68/255, 1/255, 84/255, 1.00),
    #rgb(94/255, 201/255, 98/255),
    "white",
    #rgb(59/255, 82/255, 139/255)
    rgb(253/255, 231/255, 37/255, 1.00)
    )
)
colours <-  pal(11)
