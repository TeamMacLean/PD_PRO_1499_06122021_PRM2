...
file <- here("data/raw/Nrc210121_rgm2r_1to6hrsMS1Q.xlsx" )

cleaned <- read_excel(file, sheet=excel_sheets(file)) |> 
  select(`Positions in Master Proteins`, `Modifications in Master Proteins`, `Modifications`, `Annotated Sequence`, starts_with("Abundances (Grouped):")) |> 
  pivot_longer(cols = -c(`Positions in Master Proteins`, `Modifications`, `Annotated Sequence`, `Modifications in Master Proteins`), names_to = "sample") |> 
  mutate(
...
write_csv(cleaned, here("data/cleaned/ms1_cleaned.csv"))

```

CJ: The next code chunk is adds a column `Label Modifications in Master Proteins` to meet Neftaly's and Frank's request for labels in heatmaps in subsequent analysis. 
...
cleaned <- read_excel(file, sheet=excel_sheets(file)) |> 
  ...
  separate(sample, c('time', 'sample'), sep=", ",convert=TRUE) |> 
  distinct() %>% 
  mutate(`Label Modifications in Master Proteins` = str_extract_all(`Modifications in Master Proteins`, "\\[(.*)\\]", simplify = TRUE) %>%
   gsub("\\[|\\]", "", .) %>%
  gsub("\\s*\\([^\\)]+\\)", "", .) %>%
  str_replace(., "N-Term; ", "") %>%
  str_replace(., "M([0-9]+); ", "") %>%
  ...
  mutate(`Label Modifications in Master Proteins` = ifelse(is.na(`Label Modifications in Master Proteins`), gsub("\\[|\\]", "", str_extract_all(`Modifications`, "\\[(.*)\\]", simplify = TRUE)), `Label Modifications in Master Proteins`))

write_csv(cleaned, here("data/cleaned/010_ms1_cleaned.csv"))
```
...
cleaned =  read_excel(file, sheet=excel_sheets(file)) |> 
  ...
  separate(sample, c('time', 'sample'), sep=", ",convert=TRUE) |> 
  distinct() %>% 
  # remove 1xMet-loss [N-Term]; 
  mutate(`Modifications in Master Proteins` = gsub("1xMet-loss \\[N-Term\\]; ", "", x = `Modifications in Master Proteins`)) %>%
  mutate(`Label Modifications in Master Proteins` = ifelse(!is.na(`Modifications in Master Proteins`), str_extract_all(`Modifications in Master Proteins`, "\\[.*\\]", simplify = TRUE), str_extract_all(`Modifications`, "\\[.*\\]", simplify = TRUE)) %>%
  gsub("\\[|\\]", "", .) %>%
  gsub("\\s*\\([^\\)]+\\)", "", .) %>%
  str_replace_all(., "N-Term; ", "") %>%
  str_replace_all(., "M([0-9]+); ", "") %>%
  str_replace_all(., "1xPhospho ", "") %>%
  str_replace_all(., "2xPhospho ", "")) %>%
  mutate(`Label Modifications in Master Proteins` = ifelse(is.na(`Label Modifications in Master Proteins`), gsub("\\[|\\]", "", str_extract_all(`Modifications`, "\\[(.*)\\]", simplify = TRUE)), `Label Modifications in Master Proteins`))
```
...
The long data frame needs pivot-ing into a matrix. Scaling is applied too. 
...
make_csv <- function(mat,km,rows) {
  for (i in 1:max(km$cluster)){
    ...
    file <- here("results", paste("010_kmeans_cluster_", i, "_on_guy11.csv"))
    df <- as.data.frame(tm)
    df$id = rows[rnames]
    write_csv(df, file)
  }
...
make_csv <- function(mat,km,rows) {
  for (i in 1:max(km$cluster)){
    ...
    file <- here("results", paste("010_kmeans_cluster_", i, "_on_guy11_and_dpmk1.csv"))
    df <- as.data.frame(tm)
    df$id = rows[rnames]
    write_csv(df, file)
  }
